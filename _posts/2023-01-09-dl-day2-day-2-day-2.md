...
(a) 위에서 제시된 연립 방정식을 만족하는 벡터 $\mathbf{v}=\begin{bmatrix} x\_1 \\\\ x\_2 \\\\ x\_3 \end{bmatrix} \in \mathbb{R}^3$의 집합을 구하시오.
...
(b) 행렬 $A$를 벡터 $\mathbf{x} \in \mathbb{R}^2$의 왼쪽에 곱하는 것으로 정의된 선형 변환 $L\_A(\mathbf{x})=A\mathbf{x}$가 선형 변환임을 보이시오. 즉, $L\_A$가 (a)에서 정의된 선형 변환의 엄밀한 정의에 부합함을 보이시오.
