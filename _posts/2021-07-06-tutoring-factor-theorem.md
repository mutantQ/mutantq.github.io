...
이때, $p(x)$는 다항식이고, 계수가 모두 정수이기 때문에 유리근 정리를 사용할 수 있다. 최고차항의 계수 $2$의 약수로는 $\{1, 2\}$가 있고, 상수항 $3$의 약수는 $\{1, 3\}$이 있다. 따라서, $f(a)=0$을 만족하는 유리수 $a$로서 가능한 값의 모임은 $\{+\frac{1}{1}, -\frac{1}{1}, +\frac{3}{1}, -\frac{3}{1}, +\frac{1}{2}, -\frac{1}{2}, +\frac{3}{2}, -\frac{3}{2}\}\\\\=\{1, -1, 3, -3, \frac{1}{2}, -\frac{1}{2}, \frac{3}{2}, -\frac{3}{2}\}$
이다. 
...
$p(x)=(x+1)Q\_2(x)=(x+1)(x-3)Q\_1(x)\\\\=(x+1)(x-3)(x+\frac{1}{2})Q\_0(x)$

이때, $p(x)$는 3차식이므로, $Q\_0(x)$는 0차식, 즉 상수일 수밖에 없다. 따라서 $Q\_0(x)=k$라는 상수로 둘 수 있는데, 최고차항의 계수를 비교하면 $k=2$라는 결과를 얻는다.
